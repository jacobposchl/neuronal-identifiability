"""
Robustness test suite with professional statistics and figures.
"""
...
from src.population import RealisticNeuralPopulation
from src.dynamics import generate_complex_dynamics
from src.features import (extract_deformation_features, extract_pca_features,
...
def _estimate_deformation_from_latents(latent_trajectory, dt, n_neighbors=20, n_samples=200):
...
def test_information_content(n_trials=10):
    ...
    _plot_bar(methods, stats, 'results/figures/01_information_content.png', 'Mutual Information')
    return stats
...
def test_noise_robustness(n_trials=10):
    ...
    for trial in range(n_trials):
        ...
        for nl in noise_levels:
            # Add noise to deformation trajectories (not latent positions, since we use pre-computed dynamics)
    ...
    _plot_line(noise_levels, stats, 'results/figures/02_noise_robustness.png', 'Noise Robustness')
    return stats


def test_clustering_stability(n_seeds=30):
    """Test 3: Stability across K-means random seeds (with n_init=1 for true randomness)"""
    print("\n" + "="*70)
    ...
    _plot_hist(aris, 'results/figures/03_stability.png', 'Stability')
    return aris
...
def test_sufficient_statistics(n_trials=10):
    ...
    _plot_bar(list(stats.keys()), stats, 'results/figures/04_sufficient_stats.png', 'ARI')
    return stats
...
def test_independence(n_trials=10):
    ...
    _plot_comparison(true_aris, random_aris, 'results/figures/05_independence.png')
    return {'true': true_aris, 'random': random_aris}


def _plot_bar(labels, stats, path, ylabel):
    _ensure_dirs()
...
def _plot_line(x, stats, path, title):
    _ensure_dirs()
...
def _plot_hist(data, path, title):
    _ensure_dirs()
    fig, ax = plt.subplots(figsize=(9, 5))
...
def _plot_comparison(data1, data2, path):
    _ensure_dirs()
    fig, ax = plt.subplots(figsize=(9, 5))
...
def test_false_positives(n_trials=5):
    ...
    for trial in range(n_trials):
        ...
        # Generate random firing rates matching dimensions
...
def test_latent_estimation_noise(n_trials=5):
    ...
    for trial in range(n_trials):
        ...
        rot_hat, con_hat, exp_hat = _estimate_deformation_from_latents(z_hat, dt=0.001)
